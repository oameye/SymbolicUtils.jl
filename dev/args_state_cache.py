...
# Prototype a representation-preserving per-node atomic state.
# `args` remains the same SmallVec field; only a private readiness state is added.
...
helpers = r'''@inline function _argument_cache_ready(state::ArgumentCacheState)
    ...
    if value == _ARGUMENT_CACHE_PUBLISHING
        while state[] != _ARGUMENT_CACHE_READY
            yield()
        end
        return true
    end
...
@inline function _publish_argument_cache!(
    ...
    previous = Base.Threads.atomic_cas!(
        state, _ARGUMENT_CACHE_EMPTY, _ARGUMENT_CACHE_PUBLISHING
    )
    if previous == _ARGUMENT_CACHE_EMPTY
        # Only the publication winner writes the existing SmallVec backing.
        # The READY atomic store publishes that write to `arguments` readers.
        args.data = candidate.data
        state[] = _ARGUMENT_CACHE_READY
    else
        while state[] != _ARGUMENT_CACHE_READY
            yield()
        end
