...
# Preserve `args::SmallV` exactly; add private state beside it for the three lazy variants.
for old, new in [
...
for old, new in [
('ordered_override_properties(::Type{BSImpl.AddMul{T}}) where {T} = (ArgsT{T}(), 0, 0, nothing)\n', 'ordered_override_properties(::Type{BSImpl.AddMul{T}}) where {T} = (ArgsT{T}(), ArgumentCacheState(), 0, 0, nothing)\n'),
('ordered_override_properties(::Type{<:BSImpl.ArrayOp{T}}) where {T} = (ArgsT{T}(), 0, 0, nothing)\n', 'ordered_override_properties(::Type{<:BSImpl.ArrayOp{T}}) where {T} = (ArgsT{T}(), ArgumentCacheState(), 0, 0, nothing)\n'),
('ordered_override_properties(::Type{<:BSImpl.ArrayMaker{T}}) where {T} = (ArgsT{T}(), 0, 0, nothing)\n', 'ordered_override_properties(::Type{<:BSImpl.ArrayMaker{T}}) where {T} = (ArgsT{T}(), ArgumentCacheState(), 0, 0, nothing)\n')]:
    assert text.count(old) == 1
    text = text.replace(old, new)

for old, new in [
('''                             ArgsT{T}(), Z, Z, nothing)\n''', '''                             ArgsT{T}(), ArgumentCacheState(), Z, Z, nothing)\n'''),
('''                              ArgsT{T}(), Z, Z, nothing)\n''', '''                              ArgsT{T}(), ArgumentCacheState(), Z, Z, nothing)\n'''),
('''                                 get(p, :type, type), ArgsT{T}(), Z, Z, nothing)\n''', '''                                 get(p, :type, type), ArgsT{T}(), ArgumentCacheState(), Z, Z, nothing)\n''')]:
    assert text.count(old) >= 1
    text = text.replace(old, new)
...
function _materialize_addmul_arguments!(args::ArgsT{T}, state::ArgumentCacheState, coeff, dict, variant, shape, type) where {T}
    _argument_cache_ready(state) && return args
    candidate = ArgsT{T}()
    @match variant begin
        AddMulVariant.ADD => begin
            if !iszero(coeff); push!(candidate, Const{T}(coeff)); end
            for (k, v) in dict
                newterm = @match k begin
                    BSImpl.AddMul(; dict = d2, variant = v2, type, shape, metadata) && if v2 == AddMulVariant.MUL end => Mul{T}(v, d2; shape, type, metadata)
        ...
        AddMulVariant.MUL => begin
            if !_isone(coeff); push!(candidate, Const{T}(coeff)); end
            for (k, v) in dict; push!(candidate, k ^ v); end
        end
    end
    return _publish_argument_cache!(args, state, candidate)
end

function _materialize_arrayop_arguments!(args::ArgsT{T}, state::ArgumentCacheState, output_idx, expr, reduce, term, ranges) where {T}
    _argument_cache_ready(state) && return args
    candidate = ArgsT{T}()
    push!(candidate, Const{T}(output_idx)); push!(candidate, Const{T}(expr)); push!(candidate, Const{T}(reduce)); push!(candidate, Const{T}(term)); push!(candidate, Const{T}(ranges))
    return _publish_argument_cache!(args, state, candidate)
end

function _materialize_arraymaker_arguments!(args::ArgsT{T}, state::ArgumentCacheState, regions, values) where {T}
    _argument_cache_ready(state) && return args
    candidate = ArgsT{T}()
    push!(candidate, BSImpl.Const{T}(regions)); push!(candidate, BSImpl.Const{T}(values))
    return _publish_argument_cache!(args, state, candidate)
end
